...
if(node==NULL){
        ...
        if(node->left==NULL and node->right==NULL and node->data==data){
            return NULL;
        }
        if(node->left->data==data and (node->left->left==NULL or node->left->right==NULL)){
            node->left==NULL;
        }
        if(node->right->data==data and (node->right->left==NULL or node->right->right==NULL)){
            node->right==NULL;
        }
        if(node->data==data and (node->left!=NULL and node->right!=NULL)){
            node->data=find(node->right);
            node->right=remove1(node->data,node->right);
        }
        return node;
*/