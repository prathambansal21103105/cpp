...
static int v[N], tree[4*N], lz[4*N];

void push(int i, int low, int high){
    lz[2*i]+=lz[i];
    lz[2*i+1]+=lz[i];
    int mid=low+(high-low)/2;
    tree[2*i]+=lz[i]*(mid-low+1);
    tree[2*i+1]+=lz[i]*(high-mid);
    lz[i]=0;
}
...
int query(int i, int low, int high, int l, int r){
    if(low>=l && high<=r){
        ...
        push(i,low,high);
        int mid=low+(high-low)/2;
        return query(2*i,low,mid,l,r)+query(2*i+1,mid+1,high,l,r);
    }
}
void solve(){
    int n;
