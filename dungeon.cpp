...
int binary(int st, int end, int target){
    if(st<=end){
        int mid=(st+end)/2;
        if(2*mid+((mid*7))==target){
            return mid;
        }
        else if(2*mid+((mid*7))<target){
            return binary(mid+1,end,target);
        }
        else{
            return binary(st,mid-1,target);
        }
    }
    else{
        return -1;
...
void solve(){
    ...
    if(target==9){
        ...
        int ans=binary(1,min(val,target/7),target);
        if(ans==-1){
            cout<<"NO"<<endl;
        }
        else{
