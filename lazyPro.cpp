...
/*------------code-----------------*/
void buildTree(vector<int>& v, int* tree, int low, int high, int i){
    if(low==high){
        tree[i]=v[low];
        return;
    }
...
void update(vector<int>& v, int* tree, int* lazy, int low, int high, int i, int l, int r, int val){
    if(lazy[i]!=0){
        tree[i]+=lazy[i];
        if(low!=high){
            lazy[2*i]+=lazy[i];
    ...
    int mid=low+(high-low)/2;
    update(v,tree,lazy,low,mid,2*i,l,r,val);
    update(v,tree,lazy,mid+1,high,(2*i)+1,l,r,val);
    tree[i]=min(tree[2*i],tree[(2*i)+1]);
}
void solve(){
    int n;
