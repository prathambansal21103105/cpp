...
class Fraction{
    ...
    Fraction(int num, int denom){
        this->num=num;
        this->denom=denom;
        int gcdval=__gcd(this->num,this->denom);
        this->num/=gcdval;
        this->denom/=gcdval;
        check();
    }
    void check(){
        if(denom==0){
            throw "Fraction undefined";
        }
    }
    Fraction operator+(Fraction f2){
        Fraction temp(0,1);
        temp.num=(this->num*f2.denom) + (this->denom*f2.num);
        temp.denom=this->denom*f2.denom;
        int gcdval=__gcd(temp.num,temp.denom);
        temp.num/=gcdval;
        temp.denom/=gcdval;
        return temp;
    }
